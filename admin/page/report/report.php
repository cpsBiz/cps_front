...
<script>
	...
	// 첫번째 행 키워드 타이틀 설정 함수
	function setTitle(modal = false, modalReportTitle, refresh = false) {
		let title = '';
		...
		if (!refresh) document.getElementById(!modal ? 'searchTypeTitle' : 'modal-searchTypeTitle').innerHTML = !modal ? title : modalReportTitle;
	}
	...
	function renderSumRow(data, modal = false) {
		...
		sumRow.appendChild(createCell('th', data.rewardRate === '0.00' ? '0%' : data.rewardRate + '%'));
	...
	function renderTableRows(tableData, modal = false, modalSearchType = '') {
		...
		tableData.forEach(item => {
			...
			row.appendChild(createCell('td', item.REWARD_RATE === '0.00' ? '0%' : item.REWARD_RATE + '%'));
